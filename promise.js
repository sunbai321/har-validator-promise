...
 
  ...
  return new Promise(function (resolve, reject) {  //返回一个函数,这个函数是实例化了一个promise对象，
    //Promise resolver,创造并返回一个新的承诺。 resolver必须是一个功能。该resolver函数传递两个参数：resolve和reject；
    //resolve应该用一个参数来调用。如果用一个非承诺价值来调用， 那么承诺就是用这个价值来实现的。如果用承诺（A）来调用，
    //那么所返回的承诺将承担该新承诺（A）的状态。
